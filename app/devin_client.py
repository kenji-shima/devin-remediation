...
async def get_org_usage_metrics(
    *,
    ...
    """GET /v3/organizations/{org_id}/metrics/usage -- genuinely org-scoped
    (requires only ViewOrgMetrics), unlike the /v3/enterprise/... metrics
    endpoints which need enterprise-tier permission."""
    response = await client.get(
        f"{DEVIN_BASE_URL}/v3/organizations/{org_id}/metrics/usage",
        headers=_headers(api_key),
    )
    response.raise_for_status()
    return response.json()
